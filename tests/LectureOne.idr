...
vtake Z _ = Nil
vtake (S p) (x::xs) = x :: vtake p xs

total
vdrop : (n : Nat) -> Vect (n + m) a -> Vect m a
vdrop Z x = x
vdrop (S p) (_::xs) = vdrop p xs
...
--
--trMatrix : (Matrix n m a) -> (Matrix m n a)
...
--
total
myCmp : (n:Nat) -> (m:Nat) -> Cmp n m
myCmp Z Z = cmpEQ
myCmp (S p) Z = cmpGT p
myCmp Z (S p) = cmpLT p
myCmp (S k) (S p) with (myCmp k p)
  myCmp (S k) (S k) | cmpEQ = cmpEQ
  myCmp	(S k) (S (k + S y)) | cmpLT y = cmpLT y
  myCmp	(S (p + S x)) (S p) | cmpGT x = cmpGT x
...
-- from xs?
total
mvtake :  (n : Nat) -> Vect k a -> Cmp n k -> Vect n a
mvtake n (x::xs) _ = ?vrepeatl
mvtake _ _ (cmpGT x) = _|_
